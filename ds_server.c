...
enum RescOpt {RO_All, RO_Type, RO_Avail, RO_Capable, RO_capable, RO_END};
...
const RescOption rescOptions[] = {{RO_All, "All", 3}, 
									{RO_Type, "Type", 4}, 
									{RO_Avail, "Avail", 5},
									{RO_Capable, "Capable", 7},
									{RO_capable, "capable", 7}};
...
int SendResInfoAll(char extraInfo);
int SendResInfoByType(int type, ResCapacity *resReq, char extraInfo);
int SendResInfoByServer(int type, int id, ResCapacity *resReq, char extraInfo);
int SendResInfoByAvail(ResCapacity *resReq, char extraInfo);
int SendResInfoByCapacity(ResCapacity *resReq, char extraInfo);
int SendResInfoByCapacityMin(ResCapacity *resReq, char extraInfo);
int SendResInfo(char *buffer);
int SendJobsPerStateOnServer(SchedJob *sJob, int numMsgSent);
int SendJobsOnServer(char *buffer);
inline int CountJobs(SchedJob *sJob);
int SendJobCountOfServer(char *buffer);
inline int GetTotalEstRT(SchedJob *sJob);
int SendEstWTOfServer(char *buffer);
...
int main(int argc, char **argv)
{
	...
	while (strcmp(buffer, "QUIT")) {
		...
		if (!strncmp(buffer, "CNTJ", 4)) {
			...
			ret = SendJobCountOfServer(buffer);
...
void CreateResFailureLists()
{
	...
	int actNumFailures = systemInfo.rFailT->numFailues / 2;	// due to the addition of a recovery event for every failure
...
int SendResInfoAll(char extraInfo)
{
	...
	for (i = 0; i < nsTypes; i++)
		numMsgSent += SendResInfoByType(i, NULL, extraInfo);
...
int SendResInfoByType(int type, ResCapacity *resReq, char extraInfo)
{
	...
	for (i = 0; i < limit; i++) {
		...
		if (resReq && (!IsSufficientRes(resReq, server) || server->waiting))
			continue;
		numMsgSent += SendResInfoByServer(type, i, resReq, extraInfo);
	}
...
int SendResInfoByServer(int type, int id, ResCapacity *resReq, char extraInfo)
{
	...
	if (systemInfo.rFailT)
		sprintf(buffer, "%s %d %d %d %d %d %d %d %d %d %d %d %d", FindResTypeNameByType(server->type), id, server->state, 
			availTime, server->availCores, server->availMem, server->availDisk, server->failInfo.numFails, 
	...
	// '+' as suffix of RESC option (e.g., All, Type, Capable) sends extra info, such as #waiting jobs, #running jobs at the moment
	if (extraInfo == '+') {
		sprintf(buffer, "%s %d %d", buffer, 
				CountJobs(*GetSchedJobList(Waiting, server)), 
				CountJobs(*GetSchedJobList(Running, server)));
	}
...
int SendResInfoByAvail(ResCapacity *resReq, char extraInfo)
{
	...
	for (i = 0; i < nsTypes; i++)
		numMsgSent += SendResInfoByType(i, resReq, extraInfo);
...
int SendResInfoByCapacityMin(ResCapacity *resReq, char extraInfo)
{
	...
	for (i = 0; i < nsTypes && !done; i++) {
		...
		for (j = 0; j < limit && !done; j++) {
			numMsgSent += SendResInfoByServer(i, j, resReq, extraInfo);
...
int SendResInfo(char *buffer)
{
	char *str, optStr[MAX_BUF], extraInfo;
	int numMsgSent = 0;
	
	for (str = &buffer[4]; !isalpha(*str) && *str != '\0'; str++); // skip spaces between "RESC" and the next search term
	if (*str != '\0') {
		if (!strncmp(str, rescOptions[RO_All].optstr, rescOptions[RO_All].optlen)) {
			SendDataHeader();
			extraInfo = str[rescOptions[RO_All].optlen];
			numMsgSent = SendResInfoAll(extraInfo);
		}
		else
		if (!strncmp(str, rescOptions[RO_Type].optstr, rescOptions[RO_Type].optlen)) {
			int resType;
			char resTypeName[MAX_BUF] = "";
			
			sscanf(str, "%s %s", optStr, resTypeName);
			resType = FindResTypeByName(resTypeName);
			if (resType == UNDEFINED) return UNDEFINED;
			SendDataHeader();
			extraInfo = str[rescOptions[RO_Type].optlen];
			numMsgSent = SendResInfoByType(resType, NULL, extraInfo);
		}
		else
		if (!strncmp(str, rescOptions[RO_Avail].optstr, rescOptions[RO_Avail].optlen)) {
			int numFields;
			ResCapacity resReq;
			ResCapacity *maxCapacity = &systemInfo.sTypes[systemInfo.maxServType].capacity;
			...
			if (numFields < 4 || 
				...
				return UNDEFINED;
			SendDataHeader();
			extraInfo = str[rescOptions[RO_Avail].optlen];
			numMsgSent = SendResInfoByAvail(&resReq, extraInfo);
		}
		else
		if (!strncmp(str, rescOptions[RO_Capable].optstr, rescOptions[RO_Capable].optlen)) {
			int numFields;
			ResCapacity resReq;
			ResCapacity *maxCapacity = &systemInfo.sTypes[systemInfo.maxServType].capacity;
			...
			if (numFields < 4 || 
				...
				return UNDEFINED;
			SendDataHeader();
			extraInfo = str[rescOptions[RO_Capable].optlen];
			numMsgSent = SendResInfoByCapacity(&resReq, extraInfo);
		}
		else
		if (!strncmp(str, rescOptions[RO_capable].optstr, rescOptions[RO_capable].optlen)) {
			int numFields;
			...
			SendDataHeader();
			extraInfo = str[rescOptions[RO_capable].optlen];
			numMsgSent = SendResInfoByCapacityMin(&resReq, extraInfo);
		}
		else
			return UNDEFINED;
	}
